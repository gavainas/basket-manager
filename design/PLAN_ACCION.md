...
- ✅ 4.1 **Una cara por jugador** (corregido por dirección de Gabi): la foto oficial del jugador es el **retrato ilustrado por arquetipo** — el SVG procedural era el placeholder. `Avatar` con `personality` renderiza el retrato (con variante estable por persona: espejo + tono del fondo) en TODO el juego — tira del Hub, cards, ficha, planilla, pizarra, pasar lista, vestuario, eventos, mercado y perfiles rivales — y el SVG queda como generador de respaldo (galería `#retratos` y caras sin personalidad). La identidad sigue siendo una sola por jugador, ahora con el arte que pide la fantasía; el set grande por seed es la Puerta 3.
...
6. ✅ Una sola cara por jugador (el retrato ilustrado en todo el juego, con variante estable por persona; el SVG quedó de respaldo); cero emojis de cromo en la UI (quedan los tipográficos, el 🧨 del rencor como marca semántica y los emojis-contenido del chat/noticias).
